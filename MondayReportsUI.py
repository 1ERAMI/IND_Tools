...
# Import the main functions from each report module
# Use importlib for files with special characters in names
...
AndyGregReports = import_module_from_file(
    os.path.join(script_dir, "Monday_Andy&GregReports.py"), 
    "Monday_AndyGregReports"
)
FlatironsReports = import_module_from_file(
    os.path.join(script_dir, "Monday_CameronFlatironsReports.py"), 
    "Monday_CameronFlatironsReports"
)
CrumpReports = import_module_from_file(
    os.path.join(script_dir, "Monday_CamCrumpReports.py"), 
    "Monday_CamCrumpReports"
)
MalissaReports = import_module_from_file(
    os.path.join(script_dir, "Monday_MalissaReports.py"), 
    "Monday_MalissaReports"
)
...
class UnifiedReportSenderUI:
    ...
    def __init__(self):
        ...
        self.email_vars = {}
        self.report_vars = {}  # Changed to dict for multiple checkboxes
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            header_frame,
            text="📊 Monday Reports - Universal Sender",
    ...
    def run_process(self, module, selected_emails, report_name):
        """Run the report processing in a separate thread (legacy - kept for compatibility)"""
        try:
            module.main(to_emails=selected_emails, status_callback=self.status_callback)
            self.root.after(0, lambda: messagebox.showinfo("Success", f"{report_name} sent successfully!"))
            self.root.after(100, self.root.quit)
        except Exception as e:
            error_msg = f"An error occurred: {str(e)}"
            print(error_msg)
